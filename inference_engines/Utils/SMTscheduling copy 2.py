...
def createDrugInstances(drug, interval, schedule, fixedtime):
    ...
    if fixedtime == 'N':
        ...
        for x in range(interval):
            ...
            if xname > 1:
                prevDNameMax =  str(drug+str(xname-1))
...
def schedulingDrugs(interactions, drugDetails):
    ...
    for drug in drugDetails.keys():
        freq = int(float(drugDetails[drug][0][0]))
        if len(drugDetails[drug][0][1]) > 2:
            schedule = drugDetails[drug][0][1].rstrip()
            schedule = schedule.split()
        else:
            schedule = drugDetails[drug][0][1]
        fixedTime = drugDetails[drug][0][2]
        createDrugInstances(drug,freq,schedule,fixedTime )
        drugDic[drug] = freq
